    ...
    try {
      ...
      const orderItems = items.map(item => {
        ...
        return {
          ...
          size: item.size,
          price: originalPrice, // Original price column (required for reference)
          unit_price: effectivePrice, // Use discounted price if available
      ...
      if (userId && !selectedAddressId && form.saveAddress) {
        try {
          ...
          if (addressError) {
            ...
            toast.error('Order placed successfully, but failed to save address');
          } else {
            toast.success('Order placed and address saved successfully!');
          }
      ...
      // Clear cart before navigation to avoid state conflicts
      await clearCart();
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Complete Order - ₦{(getFinalTotal() + (selectedShippingZone?.price || 0)).toLocaleString()}
                  </>
                )}
              </Button>
            </form>
