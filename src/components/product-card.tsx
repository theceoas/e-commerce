...
const hasAvailableStock = (product: any) => {
  ...
  if (product?.sizes && Array.isArray(product.sizes) && product.sizes.length > 0) {
    return product.sizes.some((size: any) => (size.stock || 0) > 0);
  }
...
interface ProductCardProps {
  product: Product & {
    ...
    price_range?: string  // Price range string like "1000 - 2000" for MiniMe products
  }
...
export function ProductCard({
  ...
  // Debug log for stock status
  useEffect(() => {
    if (!isInStock && product.name === 'obi') {
      console.log('[ProductCard] Out of Stock Debug:', {
        name: product.name,
        in_stock: product.in_stock,
        sizes: product.sizes,
        isArray: Array.isArray(product.sizes),
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-lg">
        <Image
          src={(product.thumbnail_url || product.image_url || '/placeholder-product.jpg') as string}
          alt={product.name}
          width={300}
          height={533}
          className="w-full aspect-[9/16] object-cover transition-transform duration-300 bg-gray-50"
          sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, 20vw"
          loading="lazy"
          placeholder="blur"
          blurDataURL="data:image/jpeg;base64,"
          quality={60}
          unoptimized
        />
                ...
                return (
                  <>
                    ...
                    <span className="text-xs sm:text-sm text-gray-500 line-through">
                      {priceRangeText || `₦${product.price.toLocaleString()}`}
                    </span>
                  </>
                );
              } else {
